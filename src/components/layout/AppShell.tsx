...
function CommandPalette({ onClose }: { onClose: () => void }) {
  ...
  const all: CmdItem[] = [
    { kind: 'cmd',  label: 'New note',           hint: 'Create blank note',     action: () => { navigate({ to: '/editor/$noteId', params: { noteId: 'i-comp' } }); onClose(); } },
    { kind: 'cmd',  label: 'Open Dashboard',      hint: "Today's digest",        kbd: '⌘D',  action: () => { navigate({ to: '/' }); onClose(); } },
    { kind: 'cmd',  label: 'Open Editor',         hint: 'Last note',             kbd: '⌘E',  action: () => { navigate({ to: '/editor/$noteId', params: { noteId: 'i-comp' } }); onClose(); } },
    { kind: 'cmd',  label: 'Open Graph',          hint: 'Knowledge map',         kbd: '⌘G',  action: () => { navigate({ to: '/graph' }); onClose(); } },
    { kind: 'cmd',  label: 'Toggle agent panel',  hint: '',                      ai: true },
    { kind: 'cmd',  label: 'Absorb URL…',         hint: 'Paste & summarize',     ai: true },
    { kind: 'note', label: 'On compression of thought',    hint: 'ideas/ · 2m ago'  },
  ...
  const onKey = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Enter')     { e.preventDefault(); const item = filtered[sel]; item?.action?.(); onClose(); }
  };
